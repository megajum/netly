	...
	function fb_get_post($camp) {
		...
		if ((trim ( $cg_fb_page_id ) != '')) {
			...
			if (trim ( $cg_fb_from ) != 'events')
				...
				// mbasic page URL https://m.facebook.com/page_content_list_view/more/?page_id=5550296508&start_cursor=%7B%22timeline_cursor%22%3A%22AQHRW-H78LDtVeGWdY3EPzc2lZSuGSA0Gu8fj8Zs0yTBAFiNOmRey2HVkETECGSH1hmcOCKqipA3D8-duRVAfERPu_OeAUybfbllicCSFfordFc6Lj8VPgC11fmusZKv3fDj%22%2C%22timeline_section_cursor%22%3Anull%2C%22has_next_page%22%3Atrue%7D&num_to_fetch=10&surface_type=timeline
			//$cg_fb_page_feed2 = $cg_fb_page_feed = "https://mbasic.facebook.com/" . $cg_fb_page_id;
			
				$cg_fb_page_feed2 = $cg_fb_page_feed = "https://m.facebook.com/page_content_list_view/more/?page_id=" . $cg_fb_page_id . "&start_cursor=200&num_to_fetch=1&surface_type=timeline";
			...
			if ((stristr ( $exec, 'payload' ) && stristr ( $exec, $cg_fb_page_id )) || (stristr ( $exec, 'notifications.php' ) && $cg_fb_from == 'events')) { // Checks that the object is created correctly
			...
			if (1) {
				...
				foreach ( $items as $item ) {
					...
					if ($cg_fb_from == 'events') {
						$type = 'event';
					} elseif (stristr ( $item, 'videoID' ) || stristr ( $item, 'youtube.com%2Fwatch' )) {
						$type = "video";
					...
					} elseif( stristr($exec2, '"viewer_image') ){
