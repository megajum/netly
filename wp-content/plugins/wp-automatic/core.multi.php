	...
	if (isset ( $found_links ) && count ( $found_links ) > 0) {
		...
		if ($cg_ml_cnt_method == 'visual') {
			...
			if (trim ( $cg_ml_page_2_url ) != '') {
				...
				if (stristr ( $cg_ml_page_2_url, '2' ) || stristr ( $cg_ml_page_2_url, '[page_number]' )) {
					...
					// base number
					$cg_ml_page_base = get_post_meta ( $campaign->camp_id, 'cg_ml_page_base', true );
		...
		if (trim ( $finalContent ) != '') {
			...
			if (isset ( $found_links_pagination [0] ) && trim ( $found_links_pagination [0] ) != '') {
				...
				echo '<br>Pagination next page URL:' . wp_automatic_fix_relative_link ( $found_links_pagination [0], $host, $http_prefix, $the_path, $base_url );
